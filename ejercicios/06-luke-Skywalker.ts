...
import { zip, of, pluck, zipWith, tap } from 'rxjs';
...
const enponit = '/people/1/';
const getRequest = (url: string) => ajax.getJSON<any>(url);
// ==================================================================

// Realizar el llamado al URL para obtener a Luke Skywalker
getRequest(SW_API + enponit)
  .pipe(
    tap(console.log),
    switchMap((url) => zip(of(url), getRequest(url.vehicles[0]))),
    map(([people, vehicles]) => ({ vehicles, people }))
  )
...
/**
 * nota: el array de species biene vacio por defecto, el ejercicio se realizara con los vehicles
